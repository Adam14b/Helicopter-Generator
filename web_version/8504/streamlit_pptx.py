...
class Timeline:
    def __init__(self, report: Report, y_top: int, y_bottom: int, start_date: datetime, final_date: datetime, description: list[str] = [], show_start_date=True) -> None:
        ...
        if final_date < report.now_date:
            report.add_shape(MSO_SHAPE.DONUT, start_x - 6, self.timeline_y - 6, 12, 12, color=GREEN, fill=True)
            ...
            report.add_shape(MSO_SHAPE.LINE_INVERSE, now_x, self.timeline_y, max(now_x - start_x - 6, 1), 0, color=GRAY)

        elif report.now_date <= start_date:
            report.add_shape(MSO_SHAPE.DONUT, start_x - 6, self.timeline_y - 6, 12, 12, color=GRAY, fill=True, fill_color=WHITE)
